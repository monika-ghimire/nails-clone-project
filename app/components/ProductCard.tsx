    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
      ...
      <div className="aspect-square bg-linear-to-br from-sky-100 to-sky-200 flex items-center justify-center relative">
        ...
        >
          <svg
            className={`w-5 h-5 ${
              isLiked ? "text-red-500 fill-current" : "text-slate-400"
            }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
      ...
      <div className="p-4">
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          {product.colors?.slice(0, 3).map((color) => (
            <span
              key={color}
              className="rounded-full border border-slate-200 bg-slate-100 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-600"
            >
              {color}
            </span>
          ))}
